...
func TestTargetScraperScrapeOK(t *testing.T) {
	...
	runTest(acceptHeader(config.DefaultGlobalConfig.AcceptScrapeProtocols))
	protobufParsing = true
	runTest(acceptHeader([]config.ScrapeProtocol{config.PrometheusProto, config.OpenMetricsText, config.PrometheusText}))
}
...
func TestTargetScrapeScrapeCancel(t *testing.T) {
	...
	ts := &targetScraper{
		...
		client:       http.DefaultClient,
		acceptHeader: acceptHeader(config.DefaultGlobalConfig.AcceptScrapeProtocols),
	}
	ctx, cancel := context.WithCancel(context.Background())
